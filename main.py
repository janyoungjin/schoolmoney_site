...
<body>
    ...
    {df[data.columns[7:12]].to_html(index=False,escape=False).replace("<table","<table style='width=100dvw '")}
    <img style="width: 30%;" src="data:image/png;base64,{money}">
    {df[['교내장학금','교외장학금']].to_html(index=False,escape=False).replace("<table","<table style='width=100dvw '")}
    <img style="width: 30%;" src="data:image/png;base64,{result}">
    {df[['등록금','전체 장학금']].to_html(index=False,escape=False).replace("<table","<table style='width=100dvw '")}
